import {
  ...
  breakpoints,
  themeGet
} from '@fluent-windows/styles'
import { Styles, Style } from 'jss'
import { Theme } from '../styles'
import { BoxClassProps, BoxProps } from './Box.type'
...
const breakpoint = (theme: Theme): Style =>
  breakpoints({
    sm: {
      '&::-webkit-scrollbar': {
        width: 8,
...
const acrylic = (theme: Theme): ((props: BoxProps) => Style) => (props: BoxProps): Style => {
  const bg = (props.backgroundColor || props.background || props.bg) as string
  const themeBg = themeGet(`colors.${bg}`, bg)(theme)
  return {
    position: 'relative',
    overflow: 'hidden',
